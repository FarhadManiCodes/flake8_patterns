...
from .conftest import ErrorCodes, assert_error_code, assert_no_errors, assert_no_error_code


class TestEP105MultipleAssignmentUnpacking:
    """Test EP105: Multiple-Assignment Unpacking over Indexing.
    ...
    def test_detects_sequential_indexing(self, run_checker, tier1_samples):
        ...
        assert_error_code(errors, ErrorCodes.EP105)
    ...
    def test_allows_good_unpacking(self, run_checker, tier1_samples):
        ...
        assert_no_error_code(errors, ErrorCodes.EP105)
...
y = point[1]    # Should trigger EP105
...
z = item[2]     # Non-sequential - should not trigger EP105
'''
        errors = run_checker(code)
        assert_no_error_code(errors, ErrorCodes.EP105)
    ...
    def test_single_indexing_ignored(self, run_checker):
        ...
        code = '''
item = (1, 2, 3)
x = item[0]     # Single access - should not trigger EP105
'''
        errors = run_checker(code)
        assert_no_error_code(errors, ErrorCodes.EP105)


class TestEP213ContextAwareStringConcatenation:
    """Test EP213: Context-Aware String Concatenation.
    ...
    def test_detects_missing_comma_in_list(self, run_checker, tier1_samples):
        ...
        assert_error_code(errors, ErrorCodes.EP213)
...
'''
    ...
    def test_tuple_pattern(self, run_checker):
        ...
        code = '''
config = (
    "host" "port",    # Missing comma - should trigger EP213
    "database",
)
'''
        errors = run_checker(code)
    ...
    def test_intentional_concatenation_outside_collections(self, run_checker):
        ...
        code = '''
# Intentional concatenation - should not trigger EP213
message = "Hello " "World"
'''
        errors = run_checker(code)
        assert_no_error_code(errors, ErrorCodes.EP213)
...
class TestEP318ParallelIterationWithZip:
    """Test EP318: Parallel Iteration with zip().
    ...
    def test_detects_manual_parallel_iteration(self, run_checker, tier1_samples):
        ...
        assert_error_code(errors, ErrorCodes.EP318)
    ...
    def test_allows_zip_iteration(self, run_checker, tier1_samples):
        ...
        assert_no_error_code(errors, ErrorCodes.EP318)
...
matrix2 = [[5, 6], [7, 8]]
result = []
for i in range(len(matrix1)):       # Should trigger EP318
    row = []
...
'''
    ...
    def test_index_actually_needed(self, run_checker):
        ...
        code = '''
items = ["a", "b", "c"]
for i in range(len(items)):     # Index is used - should not trigger EP318
...
'''
    ...
    def test_single_sequence_access(self, run_checker):
        ...
        code = '''
items = [1, 2, 3]
for i in range(len(items)):     # Single sequence - should not trigger EP318
    print(items[i])
'''
        errors = run_checker(code)
        assert_no_error_code(errors, ErrorCodes.EP318)
...
class TestEP320LoopVariablesAfterLoopEnds:
    """Test EP320: Loop Variables After Loop Ends.
    ...
    def test_detects_post_loop_variable_usage(self, run_checker, tier1_samples):
        ...
        assert_error_code(errors, ErrorCodes.EP320)
    ...
    def test_allows_defensive_patterns(self, run_checker, tier1_samples):
        ...
        assert_no_error_code(errors, ErrorCodes.EP320)
...
def find_large_number(numbers):
    ...
    print(f"Found: {num}")      # Should trigger EP320 - what if numbers is empty?
'''
        errors = run_checker(code)
        assert_error_code(errors, ErrorCodes.EP320)
...
print(f"Final counter: {i}")    # Intentional - should not trigger EP320
'''
        errors = run_checker(code)
        assert_no_error_code(errors, ErrorCodes.EP320)


class TestEP321DefensiveIterationOverArguments:
    """Test EP321: Be Defensive when Iterating over Arguments.
    ...
    def test_detects_multiple_iteration(self, run_checker, tier1_samples):
        ...
        assert_error_code(errors, ErrorCodes.EP321)
    ...
    def test_allows_defensive_conversion(self, run_checker, tier1_samples):
        ...
        assert_no_error_code(errors, ErrorCodes.EP321)
...
def analyze_data(values):
    ...
    maximum = max(values)       # Second iteration - should trigger EP321
...
def process_items(items):
    ...
    mapped = [x*2 for x in items]               # Second iteration - should trigger EP321
...
class TestEP426ComprehensiveDictGetPatterns:
    """Test EP426: Comprehensive dict.get() Patterns.
    ...
    def test_detects_try_except_keyerror(self, run_checker, tier1_samples):
        ...
        assert_error_code(errors, ErrorCodes.EP426)
    ...
    def test_allows_dict_get(self, run_checker, tier1_samples):
        ...
        assert_no_error_code(errors, ErrorCodes.EP426)
...
if key not in my_dict:
    my_dict[key] = []           # Should trigger EP426 - use setdefault
...
try:
    ...
    log_missing_key(key)        # Additional processing - should not trigger EP426
    notify_admin(key)
...
'''
    ...
    def test_nested_dict_access(self, run_checker):
        ...
        code = '''
try:
    host = config['database']['host']       # Should trigger EP426
except KeyError:
    host = 'localhost'
'''
        errors = run_checker(code)
...
class TestTier1Integration:
    ...
    def test_comprehensive_bad_code_detection(self, run_checker, comprehensive_bad_code):
        ...
        expected_codes = [
            ErrorCodes.EP105, ErrorCodes.EP213, ErrorCodes.EP318,
            ErrorCodes.EP320, ErrorCodes.EP321, ErrorCodes.EP426
        ]
